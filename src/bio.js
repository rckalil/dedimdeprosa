...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const jogadorSalvo = localStorage.getItem('ficha.jogador') || "";
    const folegoSalvo = localStorage.getItem('ficha.folego') || "6";
    const biografiaSalva = localStorage.getItem('ficha.biografia') || "";

    document.getElementById('nomePersonagem').value = nomeSalvo;
    document.getElementById('nomeJogador').value = jogadorSalvo;
...
function validateAndSaveInicio(direction) {
    ...
    const folego = parseInt(document.getElementById('folego').value, 12);
    ...
    localStorage.setItem('ficha.jogador', jogador);
